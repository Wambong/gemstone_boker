...
class PayClass():
    ...
    collections_subkey = "5b91fac41e1a4377bac6fccb3f076053"

    # Disbursement subscription key
    disbursements_subkey = "cb158831c6cd47a190ffab09e1ec69e9"
    ...
    if environment_mode == "sandbox":
        api_key_collections = response.get("apiKey", "")
    ...
    # Auto generate when in test mode
    if environment_mode == "sandbox":
        api_key_disbursements = response.get("apiKey", "")
    ...
    def momotokendisbursement(self):
    ...
    def momobalancedisbursement(self):
    ...
    def withdrawmtnmomo(self, amount, currency, txt_ref, phone_number, payermessage):
        # UUID V4 generator
        uuidgen = str(uuid.uuid4())
        ...
        headers = {
            ...
            'Ocp-Apim-Subscription-Key': PayClass.disbursements_subkey,
            'Content-Type': 'application/json',
            'Authorization': "Bearer "+str(self.momotokendisbursement()["access_token"])
        }

        response = requests.request("POST", url, headers=headers, data=payload)
        print(f"this is the response2: {response} ")
        context = {"response": response.status_code, "ref": uuidgen}
